...
public class Parser_2<T extends A_Model<T>> {
	...
	public Parser_2(Solver<T> solver,
                  boolean debug) {
...
//            }
	...
	public String addOperation(PrintConstraint constraint) {
		...
		if (debug) {
			...
			String opInfo = String.format("Operation: %s | %s | %d | %s", string, actualVal, id, sourceMap);
	...
	/**
	 * Assert a predicate on a symbolic value from the following boolean function:
	 * <ul>
