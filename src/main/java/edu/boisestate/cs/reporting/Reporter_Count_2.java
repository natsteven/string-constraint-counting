...
public class Reporter_Count_2<T extends A_Model<T>> extends A_Reporter_2<T> {
	...
	protected void calculateStats(PrintConstraint constraint) {
		...
		if (solver.isSingleton(base, actualVal) && (sourceMap.get("s1") == null || argIsSingleton)) {
			isSingleton = true;
		}

		// System.out.printf("%d Predicate: %s\n", base,
		// constraint.getSplitValue().split("!!")[0]);
		...
		// System.out.printf("Asserting Predicate to determine disjoint branches for
		// Constraint %d\n", base);
		...
		// System.out.printf("Asserting Negation of Predicate to determine disjoint
		// branches for Constraint %d\n", base);
